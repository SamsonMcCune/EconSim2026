...
pub fn build_relationships_with_memory<R: Rng + ?Sized>(
    ...
    let mut relationships = build_relationships_bruteforce(agents, rng);
...
impl ContinuousWorld {
    ...
    pub fn step(&mut self, dt: f64) -> StepStats {
        ...
        // 1. Production is continuous: each agent gathers a dt-scaled amount every step.
        for agent in &mut self.agents {
            produce_dt(agent, &self.resource_grid, dt);
        }

        // 2. Learning is a per-agent stochastic process rather than a synchronized sweep.
        ...
        // 3. Trade opportunities arrive continuously for eligible local pairs.
        ...
        // 4. Eating, metabolism, aging, and death are continuous. Death is checked every dt.
        ...
        // 5. Rebuild local relationships after deaths and update surplus memories continuously.
        self.rebuild_relationships_preserving_memory();
        ...
        // 6. Marriage formation is now a hazard process, not a once-per-round batch.
        ...
        // 7. Births are single-child stochastic events. High surplus/fitness increases the rate,
